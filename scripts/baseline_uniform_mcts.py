...
Reward inside MCTS rollouts is plain sparse reward for clean reward modes:
step_penalty plus terminal success.  Legacy PBRS term-similarity shaping is
used only if ``config.reward_mode == "legacy" and use_reward_shaping``.
Factor/library/on-path bonuses are intentionally excluded.  We report two
return values per target:
...
def _step_reward(
    ...
    """Step reward for the plain MCTS baseline."""
    if is_success:
        terminal = (
        ...
        return config.step_penalty + terminal
    if config.reward_mode != "legacy" or not config.use_reward_shaping:
...
def solve_uniform_mcts(
    ...
    return {
        ...
        "success_only_return": (
            float(config.success_reward if config.reward_mode == "legacy"
                  else config.terminal_success_reward)
            if success
            else 0.0
        ),
