...
# Graph construction
# ------------------------------
def build_game_graph(C: int) -> nx.DiGraph:
    """
    ...
    # Seed
    start_expr = x
    start_key = canon_key(start_expr)
    G.add_node(start_key, expr=start_expr, key=start_key, step=0, label=str(start_expr))

    # Layered BFS by "step"
    levels: Dict[int, list[str]] = {0: [start_key]}
    all_seen_by_step: Dict[int, list[str]] = {0: [start_key]}
    ...
    for t in range(C):
        ...
        if next_level:
            ...
            all_seen_by_step[t+1] = list(set(all_seen_by_step[t+1] + next_level))
        else:
