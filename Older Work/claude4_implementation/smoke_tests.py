...
try:
    ...
    from verification import (
        CircuitVerifier,
        verify_circuit_comprehensive,
        verify_circuit_quick,
    )
    from mcts_integration import (
        MCTSCircuitSolver,
        MCTSConfig,
        load_model_and_create_solver,
    )
...
class SmokeTestSuite:
    ...
    def _test_monomial_generation(self) -> bool:
        ...
        n, d = 3, 6
        index_to_monomial, monomial_to_index, _ = (
            generate_monomials_with_additive_indices(n, d)
        )
    ...
    def _test_benchmark_generators(self) -> bool:
        ...
        benchmarks = PolynomialBenchmarks(
            self.config, self.index_to_monomial, self.monomial_to_index
        )
...
def run_quick_verification():
    ...
    try:
        ...
        if len(poly_vec) != expected_size:
            ...
            if len(poly_vec) < expected_size:
                poly_vec = torch.cat(
                    [poly_vec, torch.zeros(expected_size - len(poly_vec))]
                )
            else:
                poly_vec = poly_vec[:expected_size]

        game = Game(
            poly_sp, poly_vec.unsqueeze(0), config, index_to_monomial, monomial_to_index
        )
        node = MCTSNode(game)
        legal_actions = node.get_legal_actions()
