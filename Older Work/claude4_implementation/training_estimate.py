...
class TrainingEstimator:
    ...
    def estimate_model_parameters(self) -> Tuple[int, float]:
        ...
        # Layer norms
        gnn_params += (
            (config.num_gnn_layers - 1) * config.hidden_dim * 2
        )  # weight + bias
        gnn_params += config.embedding_dim * 2  # final layer norm

        # Polynomial embedding
        poly_embed_params = (
            self.max_vector_size * config.embedding_dim + config.embedding_dim
        )
    ...
    def estimate_training_memory(self) -> Dict[str, float]:
        ...
        transformer_activation_per_sample = (
            max_seq_len * embedding_dim * self.config.num_transformer_layers * 4
        ) / (1024 * 1024)

        supervised_batch_memory = batch_size * (
            gnn_activation_per_sample
            + transformer_activation_per_sample
        ...
        return {
            ...
            "total_supervised": model_memory
            + optimizer_memory
            + gradient_memory
            + supervised_batch_memory,
            "total_ppo": model_memory
            + optimizer_memory
            + gradient_memory
            + ppo_batch_memory,
        }
    ...
    def estimate_training_time(self) -> Dict[str, float]:
        ...
        ppo_batches_per_epoch = np.ceil(
            ppo_samples_per_iteration / config.ppo_minibatch_size
        )
        ppo_update_time_per_iteration = (
            config.ppo_epochs
            * ppo_batches_per_epoch
            * config.ppo_minibatch_size
            * (forward_time_per_sample + backward_time_per_sample)
        )

        ppo_time_per_iteration = (
            trajectory_time_per_iteration + ppo_update_time_per_iteration
        )
        ...
        return {
            ...
            "total_training_days": (supervised_total_time + ppo_total_time)
            / (3600 * 24),
        }
    ...
    def print_detailed_estimate(self):
        ...
        print(
            f"    Total ({self.config.epochs} epochs): {time_est['supervised_total_hours']:.1f} hours"
        )
        print(f"  PPO training:")
        print(f"    Per iteration: {time_est['ppo_per_iteration_minutes']:.1f} minutes")
        print(
            f"    Total ({self.config.ppo_iterations} iterations): {time_est['ppo_total_hours']:.1f} hours"
        )
        ...
        print(f"\nSYSTEM REQUIREMENTS:")
        print(
            f"  Minimum RAM: {max(memory_est['total_ppo'], 4000):.0f} MB ({max(memory_est['total_ppo'] / 1024, 4):.1f} GB)"
        )
        print(
            f"  Recommended RAM: {max(memory_est['total_ppo'] * 1.5, 8000):.0f} MB ({max(memory_est['total_ppo'] * 1.5 / 1024, 8):.1f} GB)"
        )
