...
class CircuitBuilder(nn.Module):
    ...
    def forward(
        ...
        poly_embeddings = (
            poly_embeddings + torch.randn_like(poly_embeddings) * 1e-6
        )  # adding some random noise so the attention weights aren't uniform
        ...
        query = self.output_token.expand(
            -1, batch_size, -1
...
class CircuitDataset(Dataset):
    def __init__(
        self, index_to_monomial, monomial_to_index, max_vector_size, size=10000
    ):
    ...
    def generate_data(self, size):
        ...
        for _ in range(num_circuits):
            ...
            target_poly = torch.tensor(
                polynomials[-1], dtype=torch.float, device=device
            )
                ...
                dataset.append(
                    {
                        ...
                        "action": action_idx,
                        "value": (i - n - 1)
                        / (len(actions) - n - 1),  # Progress through operations
                    }
                )
    ...
    def actions_to_graph(self, actions):
        ...
        if len(edges) == 0:
            edge_index = (
                torch.tensor(
                    [[i, i] for i in range(n_nodes)], dtype=torch.long, device=device
                )
                .t()
                .contiguous()
            )
...
def train_supervised(model, dataset, config):
    ...
    data_loader = DataLoader(
        dataset, batch_size=config.batch_size, shuffle=True, collate_fn=circuit_collate
    )
...
def train_reinforce(model, dataset, config):
    ...
    for i in range(config.rl_episodes):
        (
            circuit_graph_dataset,
            target_poly,
            circuit_actions_dataset,
            mask_dataset,
            action_dataset,
            value,
        ) = dataset[random.randint(1, len(dataset) - 2)]
        ...
        if (i + 1) % 10 == 0:
            ...
            print(
                f"Episode {i + 1}/{config.rl_episodes}, Last reward: {rewards[-1]:.4f}"
            )
...
def evaluate_model(model, dataset, config, num_tests=100):
    ...
    for idx in test_indices:
        ...
        # Get prediction
        with torch.no_grad():
            action_logits, value_pred = model(
...
def main():
    ...
    dataset = CircuitDataset(
        index_to_monomial, monomial_to_index, max_vector_size, size=config.train_size
    )
