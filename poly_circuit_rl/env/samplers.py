...
class GenerativeInterestingPolynomialSampler:
    ...
    def __init__(
        ...
        # DAG data stored for external use (e.g. oracle mask, subgoal registry)
        self._G: Optional[Any] = None
        self._dist: Optional[Dict] = None
        self._roots: Optional[set] = None
    ...
    def _ensure_built(self, needed_steps: int) -> None:
        ...
        records, dist, roots = analyze_graph(
            G,
        ...
        # Store for external consumers (oracle mask, subgoal registry)
        self._G = G
        self._dist = dist
        self._roots = roots
    ...
    def get_dag_data(self):
        """Return (G, dist, roots) if the DAG has been built, else (None, None, None)."""
        return self._G, self._dist, self._roots
