...
# Hashable canonical key for a Poly dict (used for deduplication and library lookups).
PolyKey = Tuple
...
def poly_hashkey(poly: Poly) -> PolyKey:
    """Canonical hashable key for a Poly dict.

    Used for deduplication, factor library lookups, and set membership tests.
    Two polynomials are equal iff their hashkeys are equal.
    """
    return tuple(sorted(canonicalize(poly).items()))


def is_scalar(poly: Poly) -> bool:
    """Return True if poly is a constant (degree 0 or empty)."""
    for mono in poly:
...
def eval_poly(poly: Poly, values: Iterable[int | Fraction]) -> Fraction:
    ...
    if not poly:
        return Fraction(0)
    n_vars = max((len(mono) for mono in poly), default=0)
    assert len(vals) >= n_vars, f"Expected at least {n_vars} values, got {len(vals)}"
    total = Fraction(0)
    for mono, coeff in poly.items():
        term = coeff
