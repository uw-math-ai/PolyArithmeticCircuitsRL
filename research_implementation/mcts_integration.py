...
class MCTSCircuitSolver:
    ...
    def __init__(self, model: CircuitBuilder, config: Config, 
        ...
        # Initialize verifier
        self.verifier = CircuitVerifier(config, self.index_to_monomial, 
                                      self.monomial_to_index)
        
        # Initialize benchmark generator
        self.benchmarks = PolynomialBenchmarks(config, self.index_to_monomial,
                                             self.monomial_to_index)
    
    def solve_polynomial(self, target_poly_sp: sp.Expr, 
        ...
        # Initialize game
        game = Game(target_poly_sp, target_poly_vec.unsqueeze(0), self.config,
        ...
        success = circuit_found and len(game.actions_taken) <= self.config.max_complexity
    ...
    def evaluate_on_benchmarks(self, benchmark_names: List[str] = None,
                              max_time_per_problem: float = 30.0) -> Dict[str, SearchResult]:
        """
        ...
        if benchmark_names is not None:
            benchmarks = [(name, poly_sp, poly_vec) for name, poly_sp, poly_vec in all_benchmarks
        ...
        for name, poly_sp, poly_vec in benchmarks:
            ...
            print(f"  {status} | Actions: {result.num_actions} | Time: {result.search_time:.2f}s")
            
            if result.verification_result and not result.verification_result.is_correct:
                print(f"  Verification failed: {result.verification_result.error_message}")
        ...
        avg_actions = np.mean([r.num_actions for r in results.values() if r.circuit_found])
    ...
    def generate_training_data(self, num_games: int = 100) -> List[Tuple]:
        ...
        for i in range(num_games):
            ...
            try:
                ...
                states, action_probs, game_result = mcts_self_play_game(
                    self.model, self.config, poly_sp, poly_vec,
                    self.index_to_monomial, self.monomial_to_index,
        ...
        print(f"Generated {len(training_data)} training examples from {successful_games} successful games")
    ...
    def interactive_solver(self):
        ...
        while True:
            try:
                ...
                # Parse polynomial
                symbols_dict = {f"x{i}": self.benchmarks.symbols[i] 
                               for i in range(self.config.n_variables)}
                
                # Try to parse as benchmark name first
                benchmarks = self.benchmarks.get_all_benchmarks()
                benchmark_dict = {name: (poly_sp, poly_vec) 
                                for name, poly_sp, poly_vec in benchmarks}
                ...
                if result.verification_result:
                    ...
                    if vr.modular_checks:
                        ...
                        print(f"Modular checks: {modular_success}/{len(vr.modular_checks)}")
                    if vr.floating_point_checks:
                        fp_success = sum(vr.floating_point_checks)
                        print(f"Floating-point checks: {fp_success}/{len(vr.floating_point_checks)}")
...
def load_model_and_create_solver(model_path: str, config: Config = None,
                                mcts_config: MCTSConfig = None) -> MCTSCircuitSolver:
    """
