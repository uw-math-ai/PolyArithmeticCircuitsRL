...
class MCTSNode:
    ...
    def ucb_score(self, c_puct: float = 1.0) -> float:
        ...
        exploration = (c_puct * self.prior_prob * 
    ...
    def expand(self, action_probs: Dict[int, float]) -> None:
        ...
        for action in legal_actions:
            ...
            # Create child node
            child = MCTSNode(new_game, parent=self, action=action, prior_prob=prior_prob)
...
class MCTS:
    ...
    def _evaluate_leaf(self, node: MCTSNode) -> Tuple[Dict[int, float], float]:
        ...
        with torch.no_grad():
            ...
            if total_prob > 0:
                action_probs = {action: prob / total_prob 
        ...
        if game.exprs:
            ...
            try:
                if sp.expand(game.exprs[-1] - game.target_sp) == 0:
                    ...
                    efficiency_bonus = max(0, game.config.max_complexity - circuit_length)
    ...
    def get_action_probabilities(self, root: MCTSNode, temperature: float = 1.0) -> Dict[int, float]:
        """
...
def mcts_self_play_game(model, config, target_poly_sp, target_poly_vec,
                       index_to_monomial, monomial_to_index,
                       mcts_simulations: int = 800, temperature: float = 1.0) -> Tuple[List, List, float]:
    """
    ...
    """
    # Initialize game
    game = Game(target_poly_sp, target_poly_vec.unsqueeze(0), config, 
                index_to_monomial, monomial_to_index)
