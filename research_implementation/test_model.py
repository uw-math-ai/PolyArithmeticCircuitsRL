...
try:
    ...
    print(
        "Warning: Could not import from 'SupervisedTransformer_Debug.py', trying 'SupervisedTransformer.py'"
    )
    try:
        ...
        print(
            "Please ensure 'SupervisedTransformer_Debug.py' or 'SupervisedTransformer.py' is accessible."
        )
        exit()
    ...
    with torch.no_grad():
        for step_num in range(max_steps):
            ...
            for game, score, steps in beams:
                ...
                top_k_log_probs, top_k_local_indices = torch.topk(
                    valid_log_probs, min(beam_width, len(valid_log_probs))
                )

                for log_prob_val, local_idx in zip(
                    top_k_log_probs, top_k_local_indices
                ):
                    ...
                    new_score = (
                        score + log_prob_val.item()
                    )  # Add log probs (higher is better)
...
def print_circuit(steps, n_vars):
    ...
    for step in steps:
        ...
        print(
            f"Node {current_idx}: {op}(Node {n1}, Node {n2}) = {expr1} {op_symbol} {expr2}"
        )
...
def main():
    ...
    while True:
        ...
        try:
            ...
            game = Game(
                poly_sp, poly_vec, config, index_to_monomial, monomial_to_index
            ).to(device)
