...
class GPUTrainingEstimator:
    ...
    def estimate_aws_costs(self, training_time_hours: float = None) -> Dict[str, Dict[str, float]]:
        """
    ...
    def print_comprehensive_analysis(self):
        ...
        print(f"{'GPU':<12} {'Instance':<15} {'Speedup':<8} {'Total Time':<12} {'Days':<8} {'Memory':<8}")
        print("-" * 70)
        ...
        print(f"  Training time: {parallel_costs['training_hours']:.1f} hours ({parallel_costs['training_days']:.1f} days)")
        ...
        print(f"  💡 Best Value: {best_value[0]} ({self.gpu_specs[best_value[0]]['name']})")
        print(f"     - Training time: {gpu_times[best_value[0]]['total_days']:.1f} days")
...
def generate_aws_credits_proposal():
    ...
    print(f"  Main training: {costs['main_training_run']['cost']} ({costs['main_training_run']['hours']} on {costs['main_training_run']['instance']})")
    print(f"  Development: {costs['development_phase']['cost']} ({costs['development_phase']['duration']})")
