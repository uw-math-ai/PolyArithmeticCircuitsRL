...
class PolynomialBenchmarks:
    ...
    def power_sum(self, k: int) -> Tuple[sp.Expr, torch.Tensor]:
        ...
        poly_sp = sum(var**k for var in self.symbols)
        poly_vec = self.sympy_to_vector(poly_sp)
        return poly_sp, poly_vec
    ...
    def vandermonde_determinant(self, degree: int = 2) -> Tuple[sp.Expr, torch.Tensor]:
        ...
        size = min(n, degree + 1)
        matrix = sp.Matrix(size, size, [matrix_entries[i][j] 
                                       for i in range(size) 
                                       for j in range(size)])
    ...
    def random_sparse_polynomial(self, num_terms: int = 3, max_degree: int = 2) -> Tuple[sp.Expr, torch.Tensor]:
        """
...
def generate_benchmark_dataset(config, size: int = 100) -> List[Tuple[str, sp.Expr, torch.Tensor]]:
    """
...
def print_benchmark_info(config):
    ...
    print(f"Benchmark Suite for n_variables={config.n_variables}, max_complexity={config.max_complexity}")
    print("=" * 70)
